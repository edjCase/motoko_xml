...
module Slice {
  ...
  public class Slice<T>(
    ...
    let calculatedSliceLength : Nat = switch (sliceLength) {
      ...
      case (null) {
        if (sliceOffset >= sequenceSize) {
          Runtime.trap("Start index must be less than the sliceLength of the sequence");
        };
      ...
      case (?l) {
        if (sliceOffset + l > sequenceSize) {
          ...
          Runtime.trap("Start index + sliceLength must be less than the sliceLength of the sequence");
        };
        l;
      };
    };
    ...
    public func indexOf(item : T) : ?Nat {
      switch (sequence) {
        case (#array(a)) {
          ...
          label f for (i in Nat.range(0, a.size())) {
        ...
        case (#list(l)) List.indexOf(l, comparer, item);
    ...
    private func indexOfInternal(subset : Sequence<T>, onlyStartsWith : Bool) : ?Nat {
      ...
      label f1 for (sliceIndex in Nat.range(0, calculatedSliceLength - subsetSize + 1)) {
        // Check if the slice at the current index matches the sequence
        label f2 for (subsetIndex in Nat.range(0, subsetSize)) {
