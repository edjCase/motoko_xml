...
module {
  ...
  public func tokenize(value : Iter.Iter<Char>) : TokenizeResult {
    ...
    let tokenBuffer = List.empty<Token.Token>();
    loop {
      switch (getNext(reader)) {
        case (#ok(t)) {
          List.add(tokenBuffer, t);
        };
        case (#end) return #ok(List.toArray(tokenBuffer));
  ...
  private func getNext(reader : IterX.IterReader<Char>) : Result<Token.Token> or {
    ...
    let charBuffer = List.empty<Char>();
    loop {
      switch (reader.peek()) {
        case (null) {
        ...
        case (?c) {
          ...
          if (c == '<') {
            let token = if (List.size(charBuffer) > 0) {
              #text(Text.fromIter(List.values(charBuffer)));
            } else {
            ...
            switch (c) {
              ...
              case ('>') return #err("Unexpected character '>'");
              case (_) (); // Skip
            };
            List.add(charBuffer, c);
            let _ = reader.next();
          };
        };
      };
  ...
  private func getTagInfo(tag : TextSlice.TextSlice) : Result<Token.TagInfo> {
    ...
    let name : TextSlice.TextSlice = switch (tagTokens.next()) {
      case (null) return #err(UNEXPECTED_ERROR_MESSAGE);
      case (?t) switch (t) {
        case (#err(e)) return #err(e);
        case (#ok(t)) t;
      };
    ...
    let attributes = List.empty<Document.Attribute>();
          ...
          endsWith = #custom(
            func(iter : Iter.Iter<Char>) : ?(TextSlice.TextSlice, Nat) {
              do ? {
                ...
                (TextSlice.slice(#list(buffer), 0, null), 1);
              };
            }
          );
          parse = parseDocType;
    ...
    let (externalTypes : ?Document.ExternalTypesDefinition, internalTypes : [Document.InternalDocumentTypeDefinition]) = switch (iter.next()) {
      ...
      case (?#ok(t)) {
        switch (Text.toUpper(t.toText())) {
          ...
          case (_) return #err(UNEXPECTED_ERROR_MESSAGE);
        };
      };
    };
    #ok(
  ...
  private func parseInternalTypes(reader : IterX.IterReader<Char>) : Result<[Document.InternalDocumentTypeDefinition]> {
    let internalTypes = List.empty<Document.InternalDocumentTypeDefinition>();
    label l loop {
      skipWhitespace(reader);
      ...
      switch (r) {
        case (#err(e)) return #err(e);
        case (#ok(t)) {
          List.add(internalTypes, t);
        };
      };
    };
    #ok(List.toArray(internalTypes));
  };
  ...
  private func parseEntity(slice : TextSlice.TextSlice) : Result<Document.InternalDocumentTypeDefinition> {
    ...
    switch (iter.next()) {
      ...
      case (?#err(e)) return #err(e);
      case (?#ok(t)) return #err("Unexpected token '" # t.toText() # "'");
    };
  };
  ...
  private func parseNotationId(iter : TagTokenIterator) : Result<?Text> {
    switch (iter.next()) {
      ...
      case (?#ok(t)) {
        let ndata = t.toText();
        if (ndata != "NDATA") {
          return #err("Unexpected token '" #ndata # "'. Expected 'NDATA'");
        };
        switch (iter.next()) {
          case (null) #err(UNEXPECTED_ERROR_MESSAGE);
          case (?#err(e)) #err(e);
      ...
      case (?#ok(childSlice)) {
        if (childSlice.get(0) != '(') {
          switch (childSlice.toText()) {
            case ("EMPTY") #empty;
            case ("ANY") #any;
            case (c) return #err("Unexpected token '" # c # "'. Expected 'EMPTY', 'ANY', or '('");
          };
        } else {
  ...
  private func parseChildElement(childSlice : TextSlice.TextSlice) : Result<Document.ChildElement> {
    ...
    if (sliceWithoutOcurrance.get(lastIndex) != ')') {
      ...
      return #err("Unexpected token '" # Text.fromChar(sliceWithoutOcurrance.get(lastIndex)) # "'. Expected ')'");
    };
  ...
  private func parseChoiceOrSequence(slice : TextSlice.TextSlice) : Result<Document.ElementChoiceOrSequence> {
    let children = List.empty<Document.ChildElement>();
    let childCharacters = List.empty<Char>();
    var depth = 0;
    ...
    label f for (c in slice.toIter()) {
      let addCharacter : Bool = if (c == '(') {
        ...
        if (isChoice == null) {
          ...
          return #err("Cannot mix '|' and ',' in a single element");
        };
        switch (parseChildElement(TextSlice.slice(#list(childCharacters), 0, null))) {
          case (#err(e)) return #err(e);
          case (#ok(c)) List.add(children, c);
        };
        List.clear(childCharacters);
        false; // Skip seperator
      } else {
        true;
      };
      if (addCharacter) {
        List.add(childCharacters, c);
      };
    };
    switch (parseChildElement(TextSlice.slice(#list(childCharacters), 0, null))) {
      case (#err(e)) return #err(e);
    ...
    let childrenArray = List.toArray(children);
  ...
  private func parseAttribute(slice : TextSlice.TextSlice) : Result<Document.InternalDocumentTypeDefinition> {
      ...
      case (?#ok(t)) {
        if (t.get(0) == '(' and t.get(t.size() - 1) == ')') {
          ...
          switch (t.toText()) {
            ...
            case (c) return #err("Unexpected token '" # c # "'. Expected a valid attribute type.");
          };
        };
      };
    };
    ...
    let defaultValue = switch (iter.next()) {
      ...
      case (?#ok(t)) switch (t.toText()) {
        ...
        case (c) return #err("Unexpected token '" # c # "'. Expected '#REQUIRED', '#IMPLIED', or '#FIXED'");
      };
    };
    ...
    let type_ = switch (iter.next()) {
      ...
      case (?#ok(t)) {
        switch (t.toText()) {
          ...
          case (c) return #err("Unexpected token '" # c # "'. Expected 'PUBLIC' or 'SYSTEM'.");
        };
      };
    };
  ...
  private func parseQ(slice : TextSlice.TextSlice) : Result<Token.Token> {
    ...
    let token = switch (Text.toLower(tagInfo.name)) {
      case "xml" {
        ...
        for (attr in Iter.fromArray(tagInfo.attributes)) {
          switch (Text.toLower(attr.name)) {
            case "encoding" {
              encoding := attr.value;
            };
            case "version" {
              let versionString = switch (attr.value) {
                case (null) return #err("Version attribute specified with no value specified");
                case (?v) v;
              };
              ...
              major := switch (versionComponents.next()) {
                ...
                case (?major) switch (NatX.fromText(major)) {
                  case (null) return #err("Invalid version number '" # versionString # "'");
                  case (?m) m;
                };
              };
  ...
  public func matchAndParseSlice<T>(reader : IterX.IterReader<Char>, cases : [SliceMatchInfo<T>]) : Result<T> {
    let readCharacters = List.empty<Char>();
    var charIndex = 0;
    let reIter = {
      next : () -> ?Char = func() : ?Char {
        if (charIndex >= List.size(readCharacters)) {
          switch (reader.next()) {
            case (null) return null;
            case (?next) {
              List.add(readCharacters, next);
              charIndex += 1;
              ?next;
            };
          };
    ...
    return #err("No match found for '" # Text.fromIter(List.values(readCharacters)) # "'");
  };
  ...
  private func readUntilSuffix(suffix : TextSlice.TextSlice, iter : Iter.Iter<Char>) : ?TextSlice.TextSlice {
    do ? {
      ...
      loop {
        let next = iter.next()!;
        List.add(buffer, next);
        if (next == suffix.get(suffixIndex)) {
          suffixIndex += 1;

          if (suffixIndex >= suffix.size()) {
            // Found suffix
            return ?TextSlice.slice(#list(buffer), 0, null);
          };
        } else {
          suffixIndex := 0;
        };
        ...
        switch (charIter.next()) {
          ...
          case (?c) {
            ...
            if (c == '\"') {
              ...
              return ?#err("Unexpected character '" # Text.fromChar(c) # "'");
            } else {
              if (not inQuotes) {
